...
public class ApachePOIHelper {
	...
	public void downloadExel(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        sFilename = sFilename.replaceAll("${symbol_escape}r","").replaceAll("${symbol_escape}n","");
        if(header.contains("MSIE") || header.contains("Trident") || header.contains("Chrome")){
            sFilename = URLEncoder.encode(sFilename,"UTF-8").replaceAll("${symbol_escape}${symbol_escape}+","%20");
            ...
            response.setHeader("Content-Disposition","attachment;filename=${symbol_escape}""+sFilename + "."+dto.getVersion()+";${symbol_escape}"");
        }
